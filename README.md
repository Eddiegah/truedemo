...
- **`package-lock.json` had only ever been generated on Windows** (this project's whole dev environment) and was missing a Linux-only WASM-fallback entry for `@tailwindcss/oxide`'s platform-specific native binary. `npm install` silently tolerated the gap on every local build and every Vercel deploy; adding CI's `npm ci` (deliberately stricter, for exactly this kind of reproducibility gap) surfaced it immediately. Docker Desktop wasn't running locally to regenerate cross-platform, so a temporary one-off GitHub Actions workflow ran `npm install --package-lock-only` on the same Linux runners CI and the real pipeline already use, and the result was verified twice - once by the regen workflow's own `npm ci`, once by a full local Windows `npm ci` + build - before being trusted.
- **`GET /api/jobs/[id]` had no ownership check at all.** Any request with a valid job id got back the full record - app URL, repo URL, error messages - regardless of who owned it. Job ids are cuid()s and not practically guessable, but that's a mitigating factor, not a reason the endpoint was correct. Fixed to check the requester's session against the job's `userId`, returning `404` (not `403`) on a mismatch so an unauthorized request can't even confirm the job exists - verified against real production jobs, both the now-protected and the still-open (pre-auth, ownerless) cases.
